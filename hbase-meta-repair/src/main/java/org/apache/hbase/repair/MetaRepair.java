...
public class MetaRepair extends Configured implements org.apache.hadoop.util.Tool {
    ...
    public void repairMetadata(String tableName) throws Exception {
        ...
        for (String regionName : hdfsRegionNames) {
            ...
            Put info = MetaTableAccessor.makePutFromRegionInfo(regionInfo, EnvironmentEdgeManager.currentTime());
            info.addColumn(Bytes.toBytes(HBASE_META_FAMILY), Bytes.toBytes(HBASE_META_QUALIFIER_SN), Bytes.toBytes(sn));
            info.addColumn(Bytes.toBytes(HBASE_META_FAMILY), Bytes.toBytes(HBASE_META_QUALIFIER_SERVER), Bytes.toBytes(snSig[0] + ":" + snSig[1]));
            info.addColumn(Bytes.toBytes(HBASE_META_FAMILY), Bytes.toBytes(HBASE_META_QUALIFIER_STATE), Bytes.toBytes("OPEN"));
            table.put(info);
            i++;
        }
